...
async function checkThumbnailStatus() {
    ...
    const textEl = document.getElementById('thumbnailWarningText');
    if (!banner || !textEl) return;
    const base = (typeof ShokkerAPI !== 'undefined' && ShokkerAPI.baseUrl) ? ShokkerAPI.baseUrl : `http://localhost:${window._SHOKKER_PORT || 59876}`;
    try {
        const r = await fetch(base + '/api/thumbnail-status', { cache: 'no-store' });
        if (!r.ok) return;
        const data = await r.json();
        const missing = data.missing_count || 0;
        const exists = data.exists && data.png_count > 0;
        if (!exists || missing > 0) {
            let msg = !exists
                ? 'Thumbnail folder missing or empty - swatches may show generic previews. '
                : `${missing} thumbnail(s) missing - some swatches may be wrong or generic. `;
            msg += 'Run from V5 folder: python rebuild_thumbnails.py';
...
function clearImportedSpec() {
    ...
    if (specStatus) { specStatus.textContent = ''; specStatus.style.color = ''; }
    const clearBtn = document.getElementById('btnClearSpecMap');
    if (clearBtn) clearBtn.disabled = true;
    renderZones();
    showToast('Spec canvas cleared - zones will render on default spec');
}
...
function renderZones() {
    ...
    // ── SPEC CANVAS BANNER (Zone 0) ──────────────────────────────────────
    // Shows when an imported spec map is active (from SHOKK import)
    // This is NOT a zone - it's the background canvas underneath all zones
    if (importedSpecMapPath) {
        const specLabel = document.getElementById('importSpecMapStatus')?.textContent || 'Imported spec map';
        html += `<div id="specCanvasBanner" style="
            display:flex; align-items:center; gap:8px; padding:8px 10px; margin-bottom:6px;
            background: linear-gradient(135deg, rgba(0,200,80,0.08), rgba(0,150,200,0.06));
            border:1px solid rgba(0,200,80,0.3); border-radius:6px; font-size:11px;">
            <span style="font-size:14px;">🎯</span>
            <div style="flex:1; min-width:0;">
                <div style="font-weight:700; color:var(--accent-green); font-size:10px; letter-spacing:0.5px;">SPEC CANVAS (ZONE 0)</div>
                <div style="color:var(--text-dim); font-size:9px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;"
                     title="${escapeHtml(specLabel)}">
                    ${escapeHtml(specLabel)} - zones above override this
                </div>
            </div>
...
function renderZoneDetail(index) {
    ...
    if (zone.base || zone.finish) {
        html += `<div class="zone-finish-row zone-base-rotate-row" style="padding-left:24px; flex-direction: column; align-items: stretch;">
            <div style="display:flex; flex-wrap: wrap; gap: 8px 12px; width: 100%;">
                ...
                <div class="stack-control-group" style="flex: 1; min-width: 120px;">
                    ...
                    <input type="range" min="0" max="100" step="5" value="${Math.round((zone.baseStrength ?? 1) * 100)}"
                        ...
                        class="stack-slider" title="Base material intensity (5% steps)">
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneBaseStrength(${i}, 1)" title="+5%" style="padding:0 4px;font-size:10px;">+</button>
            ...
            <div class="stack-control-group" style="margin-top:8px; padding:6px 8px; background:var(--bg-card,#16162a); border:1px solid var(--border,#2a2a4a); border-radius:4px;">
                <span class="stack-label-mini" style="margin-bottom:4px;">Pattern on this layer</span>
                ...
                <div class="stack-control-group"><span class="stack-label-mini">Opacity</span>
                    ...
                    <input type="range" min="0" max="100" step="5" value="${zone.patternOpacity ?? 100}" oninput="setZonePatternOpacity(${i}, this.value)" class="stack-slider">
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZonePatternOpacity(${i}, 1)" title="+5%" style="padding:0 4px;font-size:10px;">+</button>
                ...
                <div class="stack-control-group"><span class="stack-label-mini">Scale</span>
                    ...
                    <input type="range" min="10" max="400" step="5" value="${Math.round((zone.scale || 1.0) * 100)}" oninput="setZoneScale(${i}, parseFloat(this.value)/100)" class="stack-slider">
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneScale(${i}, 1)" title="+1 step (5%)" style="padding:0 4px;font-size:10px;">+</button>
                ...
                <div class="stack-control-group"><span class="stack-label-mini">Rotate</span>
                    ...
                    <input type="range" min="0" max="359" step="1" value="${zone.rotation || 0}" oninput="setZoneRotation(${i}, this.value)" class="stack-slider">
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneRotation(${i}, 1)" title="+1°" style="padding:0 4px;font-size:10px;">+</button>
        ...
        html += `<div class="pattern-stack-section" style="border-top:1px solid var(--border);margin-top:6px;padding-top:6px;">
            ...
            <div style="padding:4px 8px;">
                ...
                <div class="stack-control-group" style="margin-top:4px;flex-wrap:wrap;gap:6px 10px;">
                    ...
                    <span style="font-size:9px;color:var(--text-dim);">${zone.secondBaseColor || '#ffffff'}</span></span>
                    <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;">
                        <input type="radio" name="sbColorSource${i}" value="special" ${zone.secondBaseColorSource ? 'checked' : ''} onchange="setZoneSecondBaseColorSource(${i}, '${(zone.secondBaseColorSource || (typeof MONOLITHICS !== 'undefined' && MONOLITHICS[0] ? 'mono:' + MONOLITHICS[0].id : 'mono:chameleon_fire')).replace(/'/g, "\\'")}')">
                        <span>From special</span>
                    </label>
                    ${zone.secondBaseColorSource ? `<select onchange="setZoneSecondBaseColorSource(${i}, this.value)" style="font-size:10px;padding:2px 6px;background:var(--bg-input);color:var(--text);border:1px solid var(--border);border-radius:4px;min-width:140px;">
                        <option value="">- pick special -</option>
                        ${(typeof MONOLITHICS !== 'undefined' ? MONOLITHICS : []).map(m => { const v = 'mono:' + m.id; return '<option value="' + v + '" ' + (zone.secondBaseColorSource === v ? 'selected' : '') + '>' + (m.name || m.id) + '</option>'; }).join('')}
                    </select>` : ''}
                </div>
                ...
                <div class="stack-control-group" style="margin-top:4px;">
                    ...
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneSecondBaseNoiseScale(${i}, -1)" title="-4px" style="padding:0 4px;font-size:10px;">−</button>
                ...
                <div class="stack-control-group" style="margin-top:4px;">
                    ...
                    <input type="range" min="10" max="500" step="5" value="${Math.min(500, Math.max(10, Math.round((zone.secondBaseScale ?? 1) * 100)))}"
                        ...
                        class="stack-slider" title="5% steps (Noise/Organic only)">
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneSecondBaseScale(${i}, 1)" title="+1 step (5%)" style="padding:0 4px;font-size:10px;">+</button>
                    <span class="stack-val" id="detSBScaleVal${i}">${(zone.secondBaseScale ?? 1).toFixed(2)}</span>
                    <span class="stack-label-mini" style="font-size:9px;color:var(--text-dim);margin-left:4px;">Noise/Organic only</span>
                </div>
                ${['pattern', 'pattern-vivid', 'tint'].includes(zone.secondBaseBlendMode || 'noise') ? `
                <div class="stack-control-group" style="margin-top:6px; padding:6px 8px; background:var(--bg-card,#16162a); border:1px solid var(--border,#2a2a4a); border-radius:4px;">
                    ...
                    <p style="font-size:9px;color:var(--text-dim);margin:0 0 6px 0;">Uses the selected zone pattern to control where this overlay appears.</p>
                    ...
                    <div class="stack-control-group" style="margin-top:6px;">
                        <button type="button" class="btn btn-sm" onclick="event.stopPropagation(); alignSecondBaseOverlayWithPrimaryPattern(${i})" title="Copy primary pattern position, scale, and rotation so the 2nd overlay lines up exactly (e.g. after you moved/resized the pattern on the map)">
                            ✓ Align with primary pattern
                        </button>
                    </div>
                </div>` : ''}
                ` : ''}
        ...
        html += `<div class="base-overlay-section" style="margin-top:6px;">
            ...
            <div style="padding:4px 8px;">
                ...
                <div class="stack-control-group" style="margin-top:4px;flex-wrap:wrap;gap:6px 10px;">
                    ...
                    <span style="font-size:9px;color:var(--text-dim);">${zone.thirdBaseColor || '#ffffff'}</span></span>
                    <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;"><input type="radio" name="tbColorSource${i}" value="special" ${zone.thirdBaseColorSource ? 'checked' : ''} onchange="setZoneThirdBaseColorSource(${i}, '${(zone.thirdBaseColorSource || (typeof MONOLITHICS !== 'undefined' && MONOLITHICS[0] ? 'mono:' + MONOLITHICS[0].id : 'mono:chameleon_fire')).replace(/'/g, "\\'")}')"><span>From special</span></label>
                    ${zone.thirdBaseColorSource ? `<select onchange="setZoneThirdBaseColorSource(${i}, this.value)" style="font-size:10px;padding:2px 6px;background:var(--bg-input);color:var(--text);border:1px solid var(--border);border-radius:4px;min-width:140px;"><option value="">- pick special -</option>${(typeof MONOLITHICS !== 'undefined' ? MONOLITHICS : []).map(m => { const v = 'mono:' + m.id; return '<option value="' + v + '" ' + (zone.thirdBaseColorSource === v ? 'selected' : '') + '>' + (m.name || m.id) + '</option>'; }).join('')}</select>` : ''}
                </div>
                <div class="stack-control-group" style="margin-top:4px;">
                    <span class="stack-label-mini">Strength</span>
                ...
                <div class="stack-control-group" style="margin-top:4px;align-items:flex-start;">
                    ...
                    <div style="display:flex;flex-direction:column;gap:2px;font-size:10px;">
                        <label style="display:flex;align-items:center;gap:4px;cursor:pointer;">
                            <input type="radio" name="tbBlendMode${i}" value="noise" ${(zone.thirdBaseBlendMode || 'noise') === 'noise' ? 'checked' : ''}
                                onchange="setZoneThirdBaseBlendMode(${i}, 'noise')">
                            <span>✨ Fractal Dust</span></label>
                        <label style="display:flex;align-items:center;gap:4px;cursor:pointer;">
                            <input type="radio" name="tbBlendMode${i}" value="uniform" ${zone.thirdBaseBlendMode === 'uniform' ? 'checked' : ''}
                                onchange="setZoneThirdBaseBlendMode(${i}, 'uniform')">
                            <span>⬜ Uniform</span></label>
                        <label style="display:flex;align-items:center;gap:4px;cursor:pointer;">
                            <input type="radio" name="tbBlendMode${i}" value="pattern" ${zone.thirdBaseBlendMode === 'pattern' ? 'checked' : ''}
                                onchange="setZoneThirdBaseBlendMode(${i}, 'pattern')">
                            <span>🔷 Pattern-Reactive</span></label>
                        <label style="display:flex;align-items:center;gap:4px;cursor:pointer;" title="Same as Pattern-Reactive but overlay color shows at FULL saturation - strength controls coverage, not color brightness">
                            <input type="radio" name="tbBlendMode${i}" value="pattern-vivid" ${zone.thirdBaseBlendMode === 'pattern-vivid' ? 'checked' : ''}
                                onchange="setZoneThirdBaseBlendMode(${i}, 'pattern-vivid')">
                            <span>💥 Pattern-Pop <span style="font-size:8px;color:var(--text-dim);">full color</span></span></label>
                        <label style="display:flex;align-items:center;gap:4px;cursor:pointer;" title="Overlay tints the base (max 35%) - does not fully cover">
                            <input type="radio" name="tbBlendMode${i}" value="tint" ${(zone.thirdBaseBlendMode || '') === 'tint' ? 'checked' : ''}
                                onchange="setZoneThirdBaseBlendMode(${i}, 'tint')">
                            <span>🎨 Tint (subtle)</span></label>
                    </div>
                </div>
                ${(zone.thirdBaseBlendMode || 'noise') === 'noise' ? `
                <div class="stack-control-group" style="margin-top:4px;">
                    <span class="stack-label-mini">Noise Scale</span>
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneThirdBaseNoiseScale(${i}, -1)" title="-4px" style="padding:0 4px;font-size:10px;">−</button>
                    <input type="range" min="4" max="128" step="4" value="${zone.thirdBaseNoiseScale || 24}"
                        oninput="setZoneThirdBaseNoiseScale(${i}, this.value)"
                        class="stack-slider" title="Noise scale">
                    <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneThirdBaseNoiseScale(${i}, 1)" title="+4px" style="padding:0 4px;font-size:10px;">+</button>
                    <span class="stack-val" id="detTBNSVal${i}">${zone.thirdBaseNoiseScale || 24}px</span>
                </div>` : ''}
                <div class="stack-control-group" style="margin-top:4px;">
                    <span class="stack-label-mini">Overlay Scale</span>
                ...
                ${['pattern', 'pattern-vivid', 'tint'].includes(zone.thirdBaseBlendMode || 'noise') ? `
                <div class="stack-control-group" style="margin-top:6px; padding:6px 8px; background:var(--bg-card,#16162a); border:1px solid var(--border,#2a2a4a); border-radius:4px;">
                    <span class="stack-label-mini" style="margin-bottom:4px;">React to zone pattern</span>
                    <p style="font-size:9px;color:var(--text-dim);margin:0 0 6px 0;">Uses the selected zone pattern to control where this overlay appears.</p>
                    <div class="stack-control-group" style="margin-bottom:6px;">
                    ...
                    <div class="stack-control-group" style="margin-top:6px;">
                        <button type="button" class="btn btn-sm" onclick="event.stopPropagation(); alignThirdBaseOverlayWithPrimaryPattern(${i})" title="Copy primary pattern position, scale, and rotation so the 3rd overlay lines up exactly">
                            ✓ Align with primary pattern
                        </button>
                    </div>
                </div>` : ''}
                ...
                <div class="base-overlay-section" style="margin-top:6px;">
                    ...
                    <div style="padding:4px 8px;">
                        ...
                        <div class="stack-control-group" style="margin-top:4px;flex-wrap:wrap;gap:6px 10px;">
                            ...
                            <span style="font-size:9px;color:var(--text-dim);">${zone.fourthBaseColor || '#ffffff'}</span></span>
                            <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;"><input type="radio" name="fbColorSource${i}" value="special" ${zone.fourthBaseColorSource ? 'checked' : ''} onchange="setZoneFourthBaseColorSource(${i}, '${(zone.fourthBaseColorSource || (typeof MONOLITHICS !== 'undefined' && MONOLITHICS[0] ? 'mono:' + MONOLITHICS[0].id : 'mono:chameleon_fire')).replace(/'/g, "\\'")}')"><span>From special</span></label>
                            ${zone.fourthBaseColorSource ? `<select onchange="setZoneFourthBaseColorSource(${i}, this.value)" style="font-size:10px;padding:2px 6px;background:var(--bg-input);color:var(--text);border:1px solid var(--border);border-radius:4px;min-width:140px;"><option value="">- pick special -</option>${(typeof MONOLITHICS !== 'undefined' ? MONOLITHICS : []).map(m => { const v = 'mono:' + m.id; return '<option value="' + v + '" ' + (zone.fourthBaseColorSource === v ? 'selected' : '') + '>' + (m.name || m.id) + '</option>'; }).join('')}</select>` : ''}
                        </div>
                        <div class="stack-control-group" style="margin-top:4px;">
                            <span class="stack-label-mini">Strength</span>
                        ...
                        <div class="stack-control-group" style="margin-top:4px;align-items:flex-start;">
                            <span class="stack-label-mini" style="padding-top:2px;">Blend Mode</span>
                            <div style="display:flex;flex-direction:column;gap:2px;font-size:10px;">
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;"><input type="radio" name="fbBlendMode${i}" value="noise" ${(zone.fourthBaseBlendMode || 'noise') === 'noise' ? 'checked' : ''} onchange="setZoneFourthBaseBlendMode(${i}, 'noise')"><span>✨ Fractal Dust</span></label>
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;"><input type="radio" name="fbBlendMode${i}" value="uniform" ${zone.fourthBaseBlendMode === 'uniform' ? 'checked' : ''} onchange="setZoneFourthBaseBlendMode(${i}, 'uniform')"><span>⬜ Uniform</span></label>
                                ...
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;" title="Same as Pattern-Reactive but overlay color shows at FULL saturation"><input type="radio" name="fbBlendMode${i}" value="pattern-vivid" ${zone.fourthBaseBlendMode === 'pattern-vivid' ? 'checked' : ''} onchange="setZoneFourthBaseBlendMode(${i}, 'pattern-vivid')"><span>💥 Pattern-Pop <span style="font-size:8px;color:var(--text-dim);">full color</span></span></label>
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;" title="Overlay tints the base (max 35%)"><input type="radio" name="fbBlendMode${i}" value="tint" ${(zone.fourthBaseBlendMode || '') === 'tint' ? 'checked' : ''} onchange="setZoneFourthBaseBlendMode(${i}, 'tint')"><span>🎨 Tint (subtle)</span></label>
                            </div>
                        </div>
                        ${(zone.fourthBaseBlendMode || 'noise') === 'noise' ? `
                        <div class="stack-control-group" style="margin-top:4px;">
                            <span class="stack-label-mini">Noise Scale</span>
                            <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneFourthBaseNoiseScale(${i}, -1)" title="-4px" style="padding:0 4px;font-size:10px;">−</button>
                            <input type="range" min="4" max="128" step="4" value="${zone.fourthBaseNoiseScale || 24}" oninput="setZoneFourthBaseNoiseScale(${i}, this.value)" class="stack-slider" title="Noise scale">
                            <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneFourthBaseNoiseScale(${i}, 1)" title="+4px" style="padding:0 4px;font-size:10px;">+</button>
                            <span class="stack-val" id="detFBNSVal${i}">${zone.fourthBaseNoiseScale || 24}px</span>
                        </div>` : ''}
                        <div class="stack-control-group" style="margin-top:4px;">
                            <span class="stack-label-mini">Overlay Scale</span>
                            ...
                            <input type="range" min="10" max="500" step="5" value="${Math.min(500, Math.max(10, Math.round((zone.fourthBaseScale ?? 1) * 100)))}" oninput="setZoneFourthBaseScale(${i}, parseFloat(this.value)/100)" class="stack-slider" title="5% steps (Noise/Organic only)">
                            <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneFourthBaseScale(${i}, 1)" title="+1 step (5%)" style="padding:0 4px;font-size:10px;">+</button>
                            <span class="stack-val" id="detFBScaleVal${i}">${(zone.fourthBaseScale ?? 1).toFixed(2)}</span>
                        </div>
                        ${['pattern', 'pattern-vivid', 'tint'].includes(zone.fourthBaseBlendMode || 'noise') ? `
                        <div class="stack-control-group" style="margin-top:6px; padding:6px 8px; background:var(--bg-card,#16162a); border:1px solid var(--border,#2a2a4a); border-radius:4px;">
                            <span class="stack-label-mini" style="margin-bottom:4px;">React to zone pattern</span>
                            <p style="font-size:9px;color:var(--text-dim);margin:0 0 6px 0;">Uses the selected zone pattern to control where this overlay appears.</p>
                            <div class="stack-control-group" style="margin-bottom:6px;">
                            ...
                            <div class="stack-control-group" style="margin-top:6px;">
                                <button type="button" class="btn btn-sm" onclick="event.stopPropagation(); alignFourthBaseOverlayWithPrimaryPattern(${i})" title="Copy primary pattern position, scale, and rotation so the 4th overlay lines up exactly">✓ Align with primary pattern</button>
                            </div>
                        </div>` : ''}
                        ` : ''}
                    </div>
                ...
                <div class="base-overlay-section" style="margin-top:6px;">
                    ...
                    <div style="padding:4px 8px;">
                        ...
                        <div class="stack-control-group" style="margin-top:4px;flex-wrap:wrap;gap:6px 10px;">
                            ...
                            <span style="font-size:9px;color:var(--text-dim);">${zone.fifthBaseColor || '#ffffff'}</span></span>
                            <label style="display:flex;align-items:center;gap:4px;cursor:pointer;font-size:10px;"><input type="radio" name="fifColorSource${i}" value="special" ${zone.fifthBaseColorSource ? 'checked' : ''} onchange="setZoneFifthBaseColorSource(${i}, '${(zone.fifthBaseColorSource || (typeof MONOLITHICS !== 'undefined' && MONOLITHICS[0] ? 'mono:' + MONOLITHICS[0].id : 'mono:chameleon_fire')).replace(/'/g, "\\'")}')"><span>From special</span></label>
                            ${zone.fifthBaseColorSource ? `<select onchange="setZoneFifthBaseColorSource(${i}, this.value)" style="font-size:10px;padding:2px 6px;background:var(--bg-input);color:var(--text);border:1px solid var(--border);border-radius:4px;min-width:140px;"><option value="">- pick special -</option>${(typeof MONOLITHICS !== 'undefined' ? MONOLITHICS : []).map(m => { const v = 'mono:' + m.id; return '<option value="' + v + '" ' + (zone.fifthBaseColorSource === v ? 'selected' : '') + '>' + (m.name || m.id) + '</option>'; }).join('')}</select>` : ''}
                        </div>
                        <div class="stack-control-group" style="margin-top:4px;">
                            <span class="stack-label-mini">Strength</span>
                        ...
                        <div class="stack-control-group" style="margin-top:4px;align-items:flex-start;">
                            <span class="stack-label-mini" style="padding-top:2px;">Blend Mode</span>
                            <div style="display:flex;flex-direction:column;gap:2px;font-size:10px;">
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;"><input type="radio" name="fifBlendMode${i}" value="noise" ${(zone.fifthBaseBlendMode || 'noise') === 'noise' ? 'checked' : ''} onchange="setZoneFifthBaseBlendMode(${i}, 'noise')"><span>✨ Fractal Dust</span></label>
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;"><input type="radio" name="fifBlendMode${i}" value="uniform" ${zone.fifthBaseBlendMode === 'uniform' ? 'checked' : ''} onchange="setZoneFifthBaseBlendMode(${i}, 'uniform')"><span>⬜ Uniform</span></label>
                                ...
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;" title="Same as Pattern-Reactive but overlay color shows at FULL saturation"><input type="radio" name="fifBlendMode${i}" value="pattern-vivid" ${zone.fifthBaseBlendMode === 'pattern-vivid' ? 'checked' : ''} onchange="setZoneFifthBaseBlendMode(${i}, 'pattern-vivid')"><span>💥 Pattern-Pop <span style="font-size:8px;color:var(--text-dim);">full color</span></span></label>
                                <label style="display:flex;align-items:center;gap:4px;cursor:pointer;" title="Overlay tints the base (max 35%)"><input type="radio" name="fifBlendMode${i}" value="tint" ${(zone.fifthBaseBlendMode || '') === 'tint' ? 'checked' : ''} onchange="setZoneFifthBaseBlendMode(${i}, 'tint')"><span>🎨 Tint (subtle)</span></label>
                            </div>
                        </div>
                        ${(zone.fifthBaseBlendMode || 'noise') === 'noise' ? `
                        <div class="stack-control-group" style="margin-top:4px;">
                            <span class="stack-label-mini">Noise Scale</span>
                            <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneFifthBaseNoiseScale(${i}, -1)" title="-4px" style="padding:0 4px;font-size:10px;">−</button>
                            <input type="range" min="4" max="128" step="4" value="${zone.fifthBaseNoiseScale || 24}" oninput="setZoneFifthBaseNoiseScale(${i}, this.value)" class="stack-slider" title="Noise scale">
                            <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneFifthBaseNoiseScale(${i}, 1)" title="+4px" style="padding:0 4px;font-size:10px;">+</button>
                            <span class="stack-val" id="detFifNSVal${i}">${zone.fifthBaseNoiseScale || 24}px</span>
                        </div>` : ''}
                        <div class="stack-control-group" style="margin-top:4px;">
                            <span class="stack-label-mini">Overlay Scale</span>
                            ...
                            <input type="range" min="10" max="500" step="5" value="${Math.min(500, Math.max(10, Math.round((zone.fifthBaseScale ?? 1) * 100)))}" oninput="setZoneFifthBaseScale(${i}, parseFloat(this.value)/100)" class="stack-slider" title="5% steps (Noise/Organic only)">
                            <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneFifthBaseScale(${i}, 1)" title="+1 step (5%)" style="padding:0 4px;font-size:10px;">+</button>
                            <span class="stack-val" id="detFifScaleVal${i}">${(zone.fifthBaseScale ?? 1).toFixed(2)}</span>
                        </div>
                        ${['pattern', 'pattern-vivid', 'tint'].includes(zone.fifthBaseBlendMode || 'noise') ? `
                        <div class="stack-control-group" style="margin-top:6px; padding:6px 8px; background:var(--bg-card,#16162a); border:1px solid var(--border,#2a2a4a); border-radius:4px;">
                            <span class="stack-label-mini" style="margin-bottom:4px;">React to zone pattern</span>
                            <p style="font-size:9px;color:var(--text-dim);margin:0 0 6px 0;">Uses the selected zone pattern to control where this overlay appears.</p>
                            <div class="stack-control-group" style="margin-bottom:6px;">
                            ...
                            <div class="stack-control-group" style="margin-top:6px;">
                                <button type="button" class="btn btn-sm" onclick="event.stopPropagation(); alignFifthBaseOverlayWithPrimaryPattern(${i})" title="Copy primary pattern position, scale, and rotation so the 5th overlay lines up exactly">✓ Align with primary pattern</button>
                            </div>
                        </div>` : ''}
                        ` : ''}
                    </div>
    ...
    if (zone.base || zone.finish) {
        ...
        html += `<div class="pattern-stack-section">
            <div class="pattern-stack-header">Pattern Layers (${1 + stackCount})
                ${zone.finish ? '<span style="font-size:9px;color:var(--text-dim);margin-left:4px;">(texture over special finish)</span>' : ''}
            ...
            ${canPlaceAnything ? `
            <div class="stack-control-group" style="margin-bottom:8px; padding:8px 10px; background:var(--bg-card,#16162a); border:1px solid var(--border,#2a2a4a); border-radius:4px;">
            ...
            <div class="pattern-layer-card">
                ...
                <div class="pattern-layer-card-controls">
                    ...
                    <div class="stack-control-group">
                        ...
                        <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneScale(${i}, -1)" title="-1 step (5%)" style="padding:0 4px;font-size:10px;">−</button>
                        <input type="range" min="10" max="400" step="5" value="${Math.round((zone.scale || 1.0) * 100)}"
                            oninput="setZoneScale(${i}, parseFloat(this.value)/100)"
                            class="stack-slider" title="Scale (5% steps)">
                        <button class="btn btn-sm stack-step-btn" onclick="event.stopPropagation(); stepZoneScale(${i}, 1)" title="+1 step (5%)" style="padding:0 4px;font-size:10px;">+</button>
                        <span class="stack-val" id="detScaleVal${i}">${(zone.scale || 1.0).toFixed(2)}x</span>
                    ...
                    <div class="stack-control-group">
                        <span class="stack-label-mini">Position X</span>
                        <input type="range" min="0" max="100" step="1" value="${Math.round((zone.patternOffsetX ?? 0.5) * 100)}"
                            oninput="setZonePatternOffsetX(${i}, this.value)" class="stack-slider" title="Pan pattern left/right (50 = center)">
                        <span class="stack-val" id="detPatPosXVal${i}">${Math.round((zone.patternOffsetX ?? 0.5) * 100)}%</span>
                    </div>
                    <div class="stack-control-group">
                            <span class="stack-label-mini">Position Y</span>
                            <input type="range" min="0" max="100" step="1" value="${Math.round((zone.patternOffsetY ?? 0.5) * 100)}"
                                oninput="setZonePatternOffsetY(${i}, this.value)" class="stack-slider" title="Pan pattern up/down (50 = center)">
                            <span class="stack-val" id="detPatPosYVal${i}">${Math.round((zone.patternOffsetY ?? 0.5) * 100)}%</span>
                    </div>
        ...
        // Extra pattern stack layers - available for ALL finishes (base AND special/monolithic)
        {
            (zone.patternStack || []).forEach((layer, li) => {
                html += `<div class="pattern-layer-card" data-layer-idx="${li}" data-zone-idx="${i}">
                    <div class="pattern-layer-card-header">
            ...
            if ((zone.patternStack || []).length < MAX_PATTERN_STACK_LAYERS) {
                html += `<button class="btn btn-sm stack-add-btn" onclick="addPatternLayer(${i})" title="Add pattern layer (max ${MAX_PATTERN_LAYERS_PER_ZONE} total)">+ Add Layer</button>`;
            }
        }
        html += `</div>`;
    }
    ...
    html += `<div class="zone-finish-row intensity-row-stacked" style="display:flex; flex-direction:column; gap:6px; padding:8px; background:var(--bg-dark); border:1px solid var(--border); border-radius:4px;">
        ...
        <div class="intensity-slider-row" style="margin-top:2px;">
            <label title="Master PBR Intensity - Scales overall finish effect strength">Master PBR</label>
...
function getSwatchUrl(finishId, colorHex, forceSplit) {
    ...
    return `${base}/api/swatch/${type}/${effectiveId}?color=${col}&size=48${splitMode}&v=${v}`;
}
...
function openSwatchPicker(triggerEl, type, zoneIndex, layerIndex) {
    ...
    let html = '';
    if (type === 'base' || type === 'secondBase' || type === 'thirdBase' || type === 'fourthBase' || type === 'fifthBase') {
...
function setZoneBaseStrength(index, val) {
    ...
    zones[index].baseStrength = Math.max(0, Math.min(2, (parseInt(val) || 100) / 100));
...
function setZonePatternSpecMult(index, val) {
    ...
    zones[index].patternSpecMult = Math.max(0, Math.min(2, (parseInt(val) || 100) / 100));
...
function setZoneSecondBaseColorSource(index, val) {
    pushZoneUndo('Overlay color from special', true);
...
function setZoneSecondBaseNoiseScale(index, val) {
    ...
    zones[index].secondBaseNoiseScale = Math.max(4, Math.min(128, parseInt(val) || 24));
    const v = zones[index].secondBaseNoiseScale;
...
function stepZoneSecondBaseNoiseScale(index, delta) {
    const cur = zones[index].secondBaseNoiseScale ?? 24;
    setZoneSecondBaseNoiseScale(index, Math.max(4, Math.min(128, cur + delta * 4)));
}
...
function setZoneThirdBase(index, val) {
    ...
    else { if (!zones[index].thirdBaseStrength) zones[index].thirdBaseStrength = 1.0; }
    renderZoneDetail(index);
    triggerPreviewRender();
}
...
function setZoneThirdBaseNoiseScale(index, val) {
    pushZoneUndo('Set 3rd overlay noise scale', true);
    zones[index].thirdBaseNoiseScale = Math.max(4, Math.min(128, parseInt(val) || 24));
    const v = zones[index].thirdBaseNoiseScale;
...
function stepZoneThirdBaseNoiseScale(index, delta) {
    const cur = zones[index].thirdBaseNoiseScale ?? 24;
    setZoneThirdBaseNoiseScale(index, Math.max(4, Math.min(128, cur + delta * 4)));
}
...
function alignSecondBaseOverlayWithPrimaryPattern(index) {
    const z = zones[index];
    if (!z || !z.secondBase) return;
    pushZoneUndo('', true);
    z.secondBasePatternOffsetX = z.patternOffsetX ?? 0.5;
    z.secondBasePatternOffsetY = z.patternOffsetY ?? 0.5;
    z.secondBasePatternScale = Math.max(SCALE_PATTERN_MIN, Math.min(SCALE_PATTERN_MAX, z.scale ?? 1.0));
    z.secondBasePatternRotation = ((z.rotation ?? 0) % 360 + 360) % 360;
    ...
    triggerPreviewRender();
    showToast('2nd overlay aligned with primary pattern (position, scale, rotation)');
}
...
function setZoneFourthBase(index, val) { pushZoneUndo('Set 4th overlay base'); zones[index].fourthBase = val || ''; if (!val) { zones[index].fourthBaseStrength = 0; zones[index].fourthBaseColorSource = null; } else { if (typeof val === 'string' && val.startsWith('mono:')) { zones[index].fourthBaseColorSource = val; } if (!zones[index].fourthBaseStrength) zones[index].fourthBaseStrength = 1.0; } renderZoneDetail(index); triggerPreviewRender(); }
...
function setZoneFourthBaseColor(index, val) { pushZoneUndo('', true); zones[index].fourthBaseColor = val || '#ffffff'; triggerPreviewRender(); }
...
function setZoneFourthBaseNoiseScale(index, val) {
    pushZoneUndo('Set 4th overlay noise scale', true);
    zones[index].fourthBaseNoiseScale = Math.max(4, Math.min(128, parseInt(val) || 24));
    const v = zones[index].fourthBaseNoiseScale;
...
function stepZoneFourthBaseNoiseScale(index, delta) {
    const cur = zones[index].fourthBaseNoiseScale ?? 24;
    setZoneFourthBaseNoiseScale(index, Math.max(4, Math.min(128, cur + delta * 4)));
}
...
function alignFourthBaseOverlayWithPrimaryPattern(index) {
    const z = zones[index];
    if (!z || !z.fourthBase) return;
    pushZoneUndo('', true);
    z.fourthBasePatternOffsetX = z.patternOffsetX ?? 0.5;
    z.fourthBasePatternOffsetY = z.patternOffsetY ?? 0.5;
    z.fourthBasePatternScale = Math.max(SCALE_PATTERN_MIN, Math.min(SCALE_PATTERN_MAX, z.scale ?? 1.0));
    z.fourthBasePatternRotation = ((z.rotation ?? 0) % 360 + 360) % 360;
    ...
    triggerPreviewRender();
    showToast('4th overlay aligned with primary pattern (position, scale, rotation)');
}
function setZoneFifthBase(index, val) { pushZoneUndo('Set 5th overlay base'); zones[index].fifthBase = val || ''; if (!val) { zones[index].fifthBaseStrength = 0; zones[index].fifthBaseColorSource = null; } else if (typeof val === 'string' && val.startsWith('mono:')) { zones[index].fifthBaseColorSource = val; } renderZoneDetail(index); triggerPreviewRender(); }
...
function setZoneFifthBaseColor(index, val) { pushZoneUndo('', true); zones[index].fifthBaseColor = val || '#ffffff'; triggerPreviewRender(); }
...
function setZoneFifthBaseNoiseScale(index, val) {
    pushZoneUndo('Set 5th overlay noise scale', true);
    zones[index].fifthBaseNoiseScale = Math.max(4, Math.min(128, parseInt(val) || 24));
    const v = zones[index].fifthBaseNoiseScale;
...
function stepZoneFifthBaseNoiseScale(index, delta) {
    const cur = zones[index].fifthBaseNoiseScale ?? 24;
    setZoneFifthBaseNoiseScale(index, Math.max(4, Math.min(128, cur + delta * 4)));
}
...
function alignFifthBaseOverlayWithPrimaryPattern(index) {
    const z = zones[index];
    if (!z || !z.fifthBase) return;
    pushZoneUndo('', true);
    z.fifthBasePatternOffsetX = z.patternOffsetX ?? 0.5;
    z.fifthBasePatternOffsetY = z.patternOffsetY ?? 0.5;
    z.fifthBasePatternScale = Math.max(SCALE_PATTERN_MIN, Math.min(SCALE_PATTERN_MAX, z.scale ?? 1.0));
    z.fifthBasePatternRotation = ((z.rotation ?? 0) % 360 + 360) % 360;
    ...
    triggerPreviewRender();
    showToast('5th overlay aligned with primary pattern (position, scale, rotation)');
}
/** Copy primary pattern position, scale, and rotation to 3rd base overlay so it lines up exactly. */
function alignThirdBaseOverlayWithPrimaryPattern(index) {
    const z = zones[index];
    if (!z || !z.thirdBase) return;
    pushZoneUndo('', true);
    z.thirdBasePatternOffsetX = z.patternOffsetX ?? 0.5;
    z.thirdBasePatternOffsetY = z.patternOffsetY ?? 0.5;
    z.thirdBasePatternScale = Math.max(SCALE_PATTERN_MIN, Math.min(SCALE_PATTERN_MAX, z.scale ?? 1.0));
    z.thirdBasePatternRotation = ((z.rotation ?? 0) % 360 + 360) % 360;
    ...
    triggerPreviewRender();
    showToast('3rd overlay aligned with primary pattern (position, scale, rotation)');
}
