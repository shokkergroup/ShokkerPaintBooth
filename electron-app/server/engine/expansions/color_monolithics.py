...
try:
    ...
    _CMF_PATH = os.path.join(_ROOT, "_staging", "monolithic_upgrades", "color_mono_factories_v2.py")
    if os.path.isfile(_CMF_PATH):
        _cmf_spec = importlib.util.spec_from_file_location("_staging_color_mono_factories_v2", _CMF_PATH)
        if _cmf_spec is not None and _cmf_spec.loader is not None:
            _CMF_V2 = importlib.util.module_from_spec(_cmf_spec)
            _cmf_spec.loader.exec_module(_CMF_V2)
except Exception as _cmf_ex:
    print(f"[Staging Bridge] color_monolithics v2 unavailable: {_cmf_ex}")
...
COLOR_PALETTE = {
    ...
    # Used by JS gradient/ghost/mc defs for accurate thumbnails
    "cerulean":       (0.00, 0.48, 0.65),
...
# CC SCALE (verified in-sim): 0=MIRROR/METALLISED PATH, 16=MAX GLOSS, 17-255=progressively dull/flat
# Use 17-255 range intentionally to give each material a distinct clearcoat character.
MATERIAL_TEMPLATES = {
    "gloss":    {"M": 5,   "R": 20,  "CC": 16,  "style": "solid"},   # Perfect showroom coat - max gloss
    "matte":    {"M": 5,   "R": 180, "CC": 200, "style": "solid"},   # Dull clearcoat - visibly matte (CC=0 was mirror path)
    "satin":    {"M": 15,  "R": 90,  "CC": 50,  "style": "solid"},   # Semi-gloss - slightly worn coat
    "metallic": {"M": 200, "R": 50,  "CC": 40,  "style": "flake"},   # Good coat, paint texture reads through
    "pearl":    {"M": 99,  "R": 35,  "CC": 30,  "style": "pearl"},   # Near-perfect coat, pearl depth visible
    "candy":    {"M": 200, "R": 15,  "CC": 16,  "style": "candy"},   # Max gloss - candy needs deepest wet look
    "chrome":   {"M": 250, "R": 3,   "CC": 16,  "style": "chrome"},  # No clearcoat - pure bare metal
    "flat":     {"M": 0,   "R": 230, "CC": 255, "style": "flat"},    # Dead flat - maximum dullness (CC=0 was mirror path)
}
...
def _make_gradient_spec(M=80, R=40, CC=30):  # CC=30: gradients look best with slight coat character, not pure showroom
...
def _make_colorshift_spec_v5():
    ...
    CC_range=40 is intentional: each CS duo pair gets spatially-varied CC
    across 16-56, giving some pairs a fresher coat feel and others a slightly
    lived-in character. This differentiates the 75 pairs from each other.
    """
    def spec_fn(shape, mask, seed, sm):
...
def _make_colorshift_paint_direct(r1, g1, b1, r2, g2, b2):
    """Direct RGB chameleon color shift - no HSV, no brown artifacts.
    Uses a structural noise field to simulate panel-orientation color shift.
    Color1 dominates the face (~70% of pixels), Color2 appears at glancing edges.
    ...
    def paint_fn(paint, shape, mask, seed, pm, bb):
        ...
        field = _engine.multi_scale_noise(shape, [96, 192, 48], [0.45, 0.35, 0.20], seed + 6000)
        field = np.nan_to_num(field, nan=0.0, posinf=1.0, neginf=0.0)
        # Normalize to 0-1 so both colors always show (engine may return unnormalized range)
        ...
        # Non-linear mapping: color1 = face color (dominant), color2 = flop (edge glance)
        # Power > 1 biases toward color1 (face view) which is the "normal" viewing angle
        t = np.clip(np.power(field, 2.5), 0, 1)

        # Direct RGB target computation - no HSV, no h=0 redness
        tr = r1 * (1.0 - t) + r2 * t
        tg = g1 * (1.0 - t) + g2 * t
        tb = b1 * (1.0 - t) + b2 * t
        ...
        # Micro metallic variation - very subtle brightness only (no hue shift)
        # Only apply if neither color is pure black (would look like noise on flat black)
...
def _make_multicolor_spec(M=150, R=45, CC=35):  # CC=35: multi-color patterns benefit from slightly worn coat - adds organic feel
...
# Prefer validated staging factory implementations when available.
if _CMF_V2 is not None:
    def _wrap_paint_factory(_factory):
        return lambda *args, **kwargs: _wrap_staging_paint_fn(_factory(*args, **kwargs))

    _make_solid_spec = _CMF_V2._make_solid_spec_v2
    _make_solid_paint = _wrap_paint_factory(_CMF_V2._make_solid_paint_v2)
    _make_gradient_spec = _CMF_V2._make_gradient_spec_v2
    _make_gradient_paint = _wrap_paint_factory(_CMF_V2._make_gradient_paint_v2)
    _make_colorshift_spec_v5 = _CMF_V2._make_colorshift_spec_v6
    _make_colorshift_paint_direct = _wrap_paint_factory(_CMF_V2._make_colorshift_paint_v2)
    _make_multicolor_spec = _CMF_V2._make_multicolor_spec_v2
    _make_multicolor_paint = _wrap_paint_factory(_CMF_V2._make_multicolor_paint_v2)
...
def get_color_monolithic_metadata():
    ...
    MC_DISPLAY = {
        "mc_usa_flag": ("All-American", "swirl"), "mc_rasta": ("Rasta", "swirl"),
...
def integrate_color_monolithics(engine_module):
    ...
    if _CMF_V2 is not None:
        _CMF_V2._engine = engine_module

    # Build all entries (no solid: solid color is now on the base via Base Color Mode)
    ...
    # Merge into engine registry — solid color entries omitted (use zone base + Base Color Mode instead)
    all_entries = {}
    ...
    print(f"[Color Monolithics] Loaded {counts['total']} entries: "
          f"{counts['gradient']} gradient, {counts['colorshift']} color-shift, {counts['multicolor']} multi-color")
