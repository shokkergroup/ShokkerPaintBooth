...
def get_base_overlay_alpha(shape, strength, blend_mode, noise_scale=24, seed=42,
    ...
    if blend_mode == "dust" and noise_fn is not None:
        ...
        power_val = 4.0 - (strength * 2.5)
        noise = np.power(noise, max(1.0, power_val))
        alpha = np.clip(noise * strength * 2.0, 0, 1).astype(np.float32)
        ...
        alpha = np.clip(marble * strength * 1.5, 0, 1).astype(np.float32)
        if pattern_mask is not None:
            alpha = np.clip(alpha * pattern_mask, 0, 1).astype(np.float32)

    elif blend_mode == "pattern" and pattern_mask is not None:
        alpha = np.clip(pattern_mask * strength, 0, 1).astype(np.float32)

    elif blend_mode == "tint" and pattern_mask is not None:
        alpha = np.clip(pattern_mask * strength * 0.35, 0, 1).astype(np.float32)

    elif blend_mode == "pattern_vivid" and pattern_mask is not None:
        threshold = np.clip(1.0 - float(strength), 0.0, 1.0)
        edge_width = 0.25
        ...
        if pattern_mask is not None:
            ...
            alpha = np.clip(mag * strength, 0, 1).astype(np.float32)
        else:
            alpha = np.full((H, W), strength, dtype=np.float32)
        ...
        alpha = np.clip(peaks * strength * 2.0, 0, 1).astype(np.float32)

    elif blend_mode == "pattern_contour" and pattern_mask is not None:
        # Iso-band: overlay where pattern is in a narrow band. strength = band center (0-1), band width ~0.12
        center = np.clip(float(strength), 0.0, 1.0)
        width = 0.12
        ...
        alpha = np.clip(alpha * strength * 2.0, 0, 1).astype(np.float32)
        ...
        mid = 0.5
        band = np.clip(float(strength), 0.0, 1.0) * 0.5
        low, high = mid - band, mid + band
        in_low = np.clip((low - pattern_mask.astype(np.float32)) / max(0.05, 1e-4), 0, 1)
        in_high = np.clip((pattern_mask.astype(np.float32) - high) / max(0.05, 1e-4), 0, 1)
        alpha = np.clip(np.maximum(in_low, in_high), 0, 1).astype(np.float32)
